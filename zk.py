"""zk — zettelkasten corpus operations.
...
These same four operations back both the volitional tool surface
(get_tool_schemas/handle_tool_call in __init__.py) and the automatic
recall/retain motions (prefetch/sync_turn) — "auto" is an optional
convenience over the same corpus operations, not a separate code path.

Every function takes an explicit ``root`` (the corpus directory) — this
module has no notion of a default location; the provider resolves
``root`` once at ``initialize()`` from ``hermes_home`` and passes it
through.
"""
...
def search(
    ...
    *,
    ...
    try:
        from lancedb_fts import run_fts
