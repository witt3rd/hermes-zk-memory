"""probe -- lightweight diagnostic tracing for hermes-zk-memory.

Every retain/recall decision this plugin makes is otherwise invisible:
`__init__.py`/`llm.py` only log on FAILURE (`logger.warning`), so there is
no way to see "did sync_turn actually fire", "what did the distiller
...
structured JSONL line beside the corpus for offline inspection —
    ...
    $HERMES_HOME/.zk-memory-trace.jsonl

Sibling to the corpus, not inside it (same convention as the LanceDB
index cache -- see ``lancedb_fts._index_dir``: derived/diagnostic
artifacts live beside ``zk/``, never inside it).

Best-effort by design: a trace failure must never break the retain/
recall it's describing. ``root=None`` (e.g. called before
``initialize()``) skips the file write and only logs.
"""
...
def trace(event: str, root: Optional[Path] = None, **fields: Any) -> None:
    ...
    ``root`` (the corpus root, e.g. ``self._root`` on the provider) is
    given, also best-effort appends one JSON line to the trace file
    beside it.
    """
    try:
