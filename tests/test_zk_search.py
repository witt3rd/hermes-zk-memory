"""zk.search() -- full-text recall, forcing the ripgrep fallback path.

We never rely on lancedb actually being installed/uninstalled on the test
machine: to force zk.search() through the rg-fallback branch
deterministically, we install a fake ``lancedb_fts`` module in
...
an ImportError subclass, which zk.search()'s except clause catches).
"""
...
def _force_lancedb_unavailable(monkeypatch):
    """Make ``from lancedb_fts import run_fts`` importable-but-unusable,
    always driving zk.search() into the rg fallback -- regardless of
    whether the real lancedb package happens to be installed here."""

    def _always_raises(*args, **kwargs):
        raise ImportError("lancedb not installed (test double)")

    fake_module = types.ModuleType("lancedb_fts")
    fake_module.run_fts = _always_raises
    monkeypatch.setitem(sys.modules, "lancedb_fts", fake_module)
    yield
